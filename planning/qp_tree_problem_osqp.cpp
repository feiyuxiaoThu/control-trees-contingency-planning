...
static c_float *create_c_float_array(const Eigen::VectorXd &C) {
...
QP_tree_problem_OSQP::QP_tree_problem_OSQP(const MPCModel &mpc, double u_min, double u_max)
...
Eigen::VectorXd QP_tree_problem_OSQP::call_solver() {
    // P-H
    auto P = create_csc_matrix(H.triangularView<Eigen::Upper>());
    ...
    if (strcmp(work->info->status, "solved") == 0) {
        ...
        for (auto I = 0; I < H.rows(); ++I) U_sol[I] = work->solution->x[I];
    }
