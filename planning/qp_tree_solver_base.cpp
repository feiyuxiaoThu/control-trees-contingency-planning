 ...
 * @Date: 2025-09-07 15:52:41
 * @LastEditors: puyu yu.pu@qq.com
 * @LastEditTime: 2025-09-07 23:41:56
 * @FilePath: /dive-into-contingency-planning/planning/qp_tree_solver_base.cpp
 * Copyright (c) 2025 by puyu, All Rights Reserved. 
 */

#include "planning/qp_tree_solver_base.hpp"

QP_tree_solver_base::QP_tree_solver_base(const MPCModel& mpc, double u_min, double u_max)
    : mpc(mpc), u_min_(u_min), u_max_(u_max) {}

QP_tree_joint_solver_base::QP_tree_joint_solver_base(const MPCModel& mpc, double u_min,
                                                     double u_max)
    : QP_tree_solver_base(mpc, u_min, u_max), control_bounds_in_A_(true) {}

Eigen::VectorXd QP_tree_joint_solver_base::solve(const Eigen::Vector2d & x0, const Eigen::Vector2d & xd, const Constraints & k,
                                int n_steps,
                                ...
                                const std::vector<std::vector<double>> & scaless)
{
    ...
    Lo.head(Up.rows()) = Eigen::VectorXd::Constant(Up.rows(), std::numeric_limits<double>::lowest());

    // control bounds constraints
    if(control_bounds_in_A_)
    {
        KA.block(Sextract.rows(), 0, H.rows(), H.rows()) = Eigen::MatrixXd::Identity(H.rows(), H.rows());
