 ...
 * @LastEditTime: 2025-09-07 01:58:02
...
std::shared_ptr<Pedestrian> generate_new_pedestrian(double p_crossing, uint32_t id,
    ...
    State init_state{new_x, new_y, is_forward ? 0: M_PI, 1.5};
...
bool CrossingPedestrian::is_crossing(double time, double x) {
    ...
    if (state_ == PedestrianState::MOVING) {
        if (time < crossing_time_ + crossing_duration_ - 0.5) {
...
foxglove::schemas::SceneUpdate PedestrianObserver::observe_pedestrians(
    const State& car_position, double lane_width) const {
    ...
    for (auto i = 0; i < pedestrians_.size(); ++i) {
        ...
        if (pedestrian) {
            ...
            entity.lifetime = foxglove::schemas::Duration{0, 100000000};
                ...
                cube_maker.color = foxglove::schemas::Color{0.0, 0.0, 0.0, 0.8};
                ...
                if (crossing_probability >= 0.99) {
                    ...
                    if (pedestrian->get_start_position().y < 0 &&
                        !pedestrian->is_forward_direction()) {
                ...
                crossing_prediction.color = foxglove::schemas::Color{1.0, 0.0, 0.1, crossing_probability};
                ...
                continue_prediction.color = foxglove::schemas::Color{0.0, 1.0, 0.1, 1 - crossing_probability};

                point.x = start_position.x - lane_width / 2.0 - 1;
                point.y = position.y;
                point.z = 0;
