...
struct TreePb {
    ...
    std::vector<std::vector<double>> scaless;
    int n_steps;

    static TreePb refined(const TreePb& tree, int n_steps_per_phase) {
        TreePb refined_tree;
        refined_tree.varss = std::vector<std::vector<int>>(tree.varss.size());
        refined_tree.scaless = std::vector<std::vector<double>>(tree.scaless.size());
    ...
    static std::shared_ptr<TreePb> refined(const std::shared_ptr<TreePb>& tree,
                                           int n_steps_per_phase) {
...
struct Tree2Branches2Steps : public TreePb {
    Tree2Branches2Steps(double p) {
        ...
        scaless.push_back(
            {1.0, 1.0, 1.0 - p, 1.0 - p, 1.0 - p, 1.0 - p, 1.0 - p, 1.0 - p, 1.0 - p, 1.0 - p});
